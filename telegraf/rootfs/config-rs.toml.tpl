...
# kube-system-metrics with prometheus output
{{ if .KUBERNETES_METRICS_ENABLE }}
[[inputs.prometheus]]
  # An array of urls to scrape metrics from.
  urls = ["http://kube-state-metrics.kube-system:8080/metrics"]
  # Specify timeout duration for slower prometheus clients (default is 3s)
  response_timeout = "5s"
  # pass only several metrics
  name_prefix = {{ .KUBERNETES_ENV_PREFIX | quote }}
  {{ $var_ready := printf "%s%s" .KUBERNETES_ENV_PREFIX "kube_pod_container_status_ready" }}
  {{ $var_restarts := printf "%s%s" .KUBERNETES_ENV_PREFIX "kube_pod_container_status_restarts" }}
  {{ $var_running := printf "%s%s" .KUBERNETES_ENV_PREFIX "kube_pod_container_status_running" }}
  namepass = [ {{ $var_ready | quote }}, {{ $var_restarts | quote }}, {{ $var_running | quote }} ]
{{ end }}
